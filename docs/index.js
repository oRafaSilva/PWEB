// Constante da API Key (já está OK)
const apiKey = "6c40d8fae3f27f2dabb6fc78edd4d510";

// Array para armazenar os filmes do carrossel (na memória)
let filmesNoCarrossel = []; // Este array será sincronizado com o LocalStorage
...
// O carrosselElement deve ser definido após o DOM carregar completamente
// para garantir que o elemento seja encontrado.
let carrosselElement; // Declare fora para ser acessível, mas inicialize em DOMContentLoaded

function scrollEsquerda() {
    if (!carrosselElement) { // Verifica se já foi inicializado
...
function adicionarAoCarrossel(id, titulo, imagemUrl) {
    ...
    // 1. Redirecionar para a aba "Meus Assistidos"
    abrirAba('aba-assistidos'); // Chama sua função existente para abrir a aba

    // 2. Limpar/resetar a aba de buscar filme
    ...
    if (resultadosBuscaContainer) {
        resultadosBuscaContainer.innerHTML = ""; // Limpa os resultados da busca
    }
    fecharModal(); // Se a busca estiver em um modal, feche-o também.
...
function renderizarCarrosselPessoal() {
    ...
    carrosselPessoalContainer.innerHTML = ""; // Limpa o conteúdo atual
    ...
    filmesNoCarrossel.forEach(filme => {
        ...
        `; // CORREÇÃO NA URL DO TRAILER: usei google search por youtube, que é mais confiável.
        carrosselPessoalContainer.appendChild(novoFilmeElement);
    });
}

// --- Nova Função para Remover Filme do Carrossel ---
...
function abrirAba(id) {
    ...
    // Opcional: Ativar o botão da aba correspondente
...
document.addEventListener('DOMContentLoaded', () => {
    // Inicialize carrosselElement aqui, após o DOM estar pronto
    carrosselElement = document.getElementById('carrossel');
